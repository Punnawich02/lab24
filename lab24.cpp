...
ComplexNumber operator*(double a ,const ComplexNumber &c)
{
	return ComplexNumber(c.real*a ,a*c.imag);
}

ComplexNumber ComplexNumber::operator/(const ComplexNumber &c)
{
	double re = ((real*c.real)+(imag*c.imag)) / (pow(c.real,2)+pow(c.imag,2));
	double im = (-(real*c.imag)+(imag*c.real)) / (pow(c.real,2)+pow(c.imag,2));
	return ComplexNumber(re,im);
}

ComplexNumber operator/(double a ,const ComplexNumber &c)
{
	double re = ((a*c.real)+(0*c.imag)) / (pow(c.real,2)+pow(c.imag,2));
	double im = (-(a*c.imag)+(0*c.real)) / (pow(c.real,2)+pow(c.imag,2));
	return ComplexNumber(re,im);
}
...
{
	if(a == c.real and 0 == c.imag) return 1;
	else return 0;
}
